...
def recognize_numbers():
    ...
    out1 = f"You said: {second_number}"
    output_text.insert(tk.END, out1 + "\n")
    output_text.update()
    print(out1)
    speak(out1)
    ...
    while True:
        ...
        # To play Game
            ...
            for question in questions:
                speak(question)
                output_text.insert(tk.END, question + "\n\n")
                output_text.update()
                print(question)
                answer = takeCommand()
        ...
        # tell ip address
            ...
            print(ip)
            output = f"Your ip address is {ip}"
        ...
        # other commands
            ...
            speak(myname)
            output = f"My Friends call me {myname}"
            output_text.insert(tk.END, output + "\n")
            output_text.update()
        ...
        # perform calculation
            ...
            output = f"The {operation} of {first_number} and {second_number} is {r}."
            output_text.insert(tk.END, output + "\n")
            output_text.update()
        ...
        # tell news
            ...
            output = "Top 5 news of all categories"
            output_text.insert(tk.END, output + "\n")
            output_text.update()
        ...
        # to tell weather
            ...
            output="Weather Report of Trichy City"
            output_text.insert(tk.END, output + "\n\n")
            output="Temperature:   " + str(int(data['main']['temp']))
            output_text.insert(tk.END, output + "\n\n")
            output_text.update()
        ...
        # to open code
            ...
            speak("Opeining code")
            codepath = "D:\C# project\Assignment-2\Assignment-2.sln"
...
if __name__ == '__main__':
    ...
    root.title("Sophia(Voice Assistant)")

    root.configure(bg='#A2B5CD')

    label = tk.Label(root, text='Desktop Voice Assistant', font=('Times', 15))
    label.config(bg="#BCD2EE", fg="Black")
    label.pack()

    image = Image.open("D:\Final Year project\Desktop Voice Assistant\Mic.png")
    ...
    output_text = tk.Text(root, height=20, width=55,yscrollcommand=v.set)
    # Attach the scrollbar with the text widget
    v.config(command=output_text.yview)
    output_text.pack()

    # Function to auto-scroll the Text widget
    def autoscroll():
        output_text.yview_moveto(1.0)
        root.after(1000, autoscroll)  # Call this function again after 1 second
